...
<StepGallery items={props.stepItems.filter(item => item.src.includes('rise'))} label="The Rise" client:visible /> items={props.galleryItems.filter(item => item.src.includes('slice') || item.src.includes('baked'))} label="Thermal Results" client:visible />
