...
export const doughItems = props.galleryItems.filter(item => item.src.includes('dough'));
export const sliceItems = props.galleryItems.filter(item => item.src.includes('slice') || item.src.includes('baked'));
...
<BlueprintGallery items={doughItems} label="Hydration & Mixing" client:visible />
...
<BlueprintGallery items={sliceItems} label="Thermal Results" client:visible />
